# Orquestrador Conversacional Inteligente - Visão Geral

**Objetivo:** Transformar sistema de "trilho fixo" em diálogo adaptativo onde usuário e sistema decidem caminho juntos através de negociação contínua.

**Status:** ✅ MVP Concluído (15/11/2025)

**Dependências:**
- Épico 6.2 concluído (registro de memória)

---

## Implementado

- ✅ POC (7.1-7.4): Exploração, análise contextual, sugestão, detecção de mudança
- ✅ Protótipo (7.5-7.7): CLI conversacional, detecção inteligente, transparência de raciocínio
- ✅ MVP (7.8-7.10): Argumento focal explícito, provocação de reflexão, detecção emergente de estágio

**Validação:**
- Testes em `tests/core/integration/behavior/test_orchestrator_integration.py`

---

**Versão:** 1.2 (MVP Completo)  
**Data:** 15/11/2025

**Próximas seções:**
- [Arquitetura](./architecture.md) - Decisão arquitetural e novo comportamento
- [Raciocínio](./reasoning.md) - Capacidades e análise contextual
- [Fluxo](./flow.md) - Fluxo conversacional completo
- [Critérios de Aceite](./acceptance_criteria.md) - Funcionalidades mínimas
- [Progressão](./progression.md) - POC → Protótipo → MVP
- [Implementação](./implementation.md) - Mudanças técnicas
- [Exemplos](./examples.md) - Exemplos concretos
- [Curadoria](./curation.md) - Modelo de curadoria
- [Notas](./notes.md) - Limitações e próximos passos
- [Protótipo CLI](./prototype_cli.md) - CLI Conversacional (Épico 7.5-7.7)

